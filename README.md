...
* Hidden executable files in system binary directories (`/bin`, `/usr/bin`, `/sbin`, `/usr/sbin`), where legitimate packages never install dotfiles, plus a short list of known loader filenames that skip the dotfile convention on purpose (deliberately boring names instead of hidden ones).
